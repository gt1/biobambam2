...
int bamclipextract(::libmaus::util::ArgInfo const & arginfo)
{
	...
	while ( dec.readAlignment() )
	{
		if ( algn.isMapped() )
		{
			...
			if ( 
				numcig && 
				cigop[0].first == libmaus::bambam::BamFlagBase::LIBMAUS_BAMBAM_CSOFT_CLIP &&
			...
			)
			{
				...
				front.putFlags(front.getFlags() | libmaus::bambam::BamFlagBase::LIBMAUS_BAMBAM_FUNMAP);
				front.putRefId(-1);
			...
			if ( 
				numcig > 1 && 
				cigop[numcig-1].first == libmaus::bambam::BamFlagBase::LIBMAUS_BAMBAM_CSOFT_CLIP &&
				...
				)
			{
				...
				back.putFlags(back.getFlags() | libmaus::bambam::BamFlagBase::LIBMAUS_BAMBAM_FUNMAP);
				back.putRefId(-1);
